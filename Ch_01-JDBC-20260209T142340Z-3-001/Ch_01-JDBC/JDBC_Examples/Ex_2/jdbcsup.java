 ...
 class frmrecord extends Frame implements ActionListener
{
	...
	public frmrecord()
	{
		...
		try
		{
			...
			cn=DriverManager.getConnection("jdbc:odbc:supdsn");
		}
		catch(Exception e)
		{
...
• TYPE FORWARD ONLY: the result set is non-scrollable, the cursor can be
moved using only the next() and last() methods (no methods that go
back can be used) (default).

• TYPE SCROLL INSENSITIVE: the result set is scrollable, i.e., the cursor can
be moved both forward (next(), last()) and backwards (previous(),
first()). Also, the result set typically does not change in response to
changes in the database.

• TYPE SCROLL SENSITIVE: the result set is scrollable, i.e., the cursor can
be moved both forward (next(), last()) and backwards (previous(),
first()). Also, the result set typically changes if the data in the under-
lying database changes.

In addition, the result set may, or may not be updatable. This is controlled
by the concurrency setting:

• CONCUR READ ONLY: the result set is read-only, no programmatic updates
are allowed (default).
